...
// Consent/Waiver interfaces and functions
...
export async function checkConsent(
  ...
  const params = new URLSearchParams({ email, yoga_type_id: yogaTypeId });
  return fetchAPI<ConsentCheckResult>(`/api/consent/check?${params.toString()}`);
}
...
export async function signConsent(data: {
  ...
  const res = await fetch(`${API_BASE}/api/consent/sign`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
  });
