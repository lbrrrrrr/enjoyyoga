            ...
            {yogaClass.packages && yogaClass.packages.length > 0 && (
              <div className="mt-2 space-y-1">
                ...
                {yogaClass.packages.map((pkg) => {
                  ...
                  const perSessionPrice = pkg.price / pkg.session_count;
                  const savings = yogaClass.price
                    ? ((yogaClass.price - perSessionPrice) / yogaClass.price * 100).toFixed(0)
                    : null;
                  return (
                    <div key={pkg.id} className="text-sm bg-blue-50 rounded p-2">
                      <span className="font-medium">{pkgName}</span>
                      {" - "}
                      <span>{pkg.session_count} {t("sessions")}: \u00a5{pkg.price}</span>
                      {" "}
                      <span className="text-gray-500">(\u00a5{perSessionPrice.toFixed(0)}/{t("perSession")})</span>
